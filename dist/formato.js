	...
	var Formato = (function () {
	    ...
	    _createClass(Formato, [{
	        ...
	        value: function format(number, options) {
	            ...
	            // config
	            var precision = options && options.precision ? options.precision : this.defaultConfig.precision;
	            var thousand = options && options.thousand ? options.thousand : this.defaultConfig.thousand;
	            var decimal = options && options.decimal ? options.decimal : this.defaultConfig.decimal;

	            // fixes precision
	            var toFixed = function toFixed(value, precision) {
	                precision = precision ? precision : _this.defaultConfig.precision;
	            ...
	            var integerPart = isFloat ? parseInt(toFixed(absNumber, precision), 10) + '' : absNumber + '';
	            ...
	            // format
	            return negativeSign + (greatestThousandGroupSize ? integerPart.substr(0, greatestThousandGroupSize) + thousand : '') + integerPart.substr(greatestThousandGroupSize).replace(/(\d{3})(?=\d)/g, '$1' + thousand) + (isFloat ? decimal + toFixed(absNumber, precision).split('.')[1] : '');
	        }
	    }]);
